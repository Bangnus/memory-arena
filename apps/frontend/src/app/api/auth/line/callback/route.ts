...
export async function GET(request: NextRequest) {
  ...
  const channelId = process.env.NEXT_PUBLIC_LINE_CLIENT_ID || process.env.LINE_CHANNEL_ID;
  const channelSecret = process.env.LINE_CHANNEL_SECRET;
  const callbackUrl =
    process.env.NEXT_PUBLIC_LINE_CALLBACK_URL ||
    `${new URL(request.url).origin}/api/auth/line/callback`;
  ...
  try {
    ...
    if (code.startsWith('mock_') || !channelSecret) {
    ...
    // Render mobile success confirmation view
