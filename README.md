...
Memory Arena เป็นเกมแข่งขันประลองความจำระดับมิลลิวินาทีของผู้เล่น 2 คน รองรับปุ่มสี **4 สีหลัก** (**แดง, เขียว, น้ำเงิน, เหลือง**) แข่งขันกันในรูปแบบชนะ 2 ใน 3 รอบ (**Best of 3: First to 2 Points**)

### ระดับความยาก (Difficulty Modes)
ผู้เล่นสามารถเลือกปรับระดับความยากผ่านปุ่มควบคุมบนบอร์ดหรือหน้าเว็บก่อนเริ่มเกมได้ 3 ระดับ:
* **Easy (ง่าย):** ลำดับโจทย์ 3 ขั้นตอน (ความเร็วแสดงผลดวงละ 0.8 วินาที / 800ms)
* **Medium (ปานกลาง):** ลำดับโจทย์ 4 ขั้นตอน (ความเร็วแสดงผลดวงละ 0.65 วินาที / 650ms)
* **Hard (ยาก):** ลำดับโจทย์ 6 ขั้นตอน (ความเร็วแสดงผลดวงละ 0.45 วินาที / 450ms)

### กติการอบแข่งขันและกรณีพิเศษ (Round Rules & Edge Cases)
1. **เฟสแสดงโจทย์ (Sequence Phase):** ผู้เล่นต้องจดจำสัญญาณไฟตามจังหวะ **ห้ามกดปุ่มตอบในขณะที่สัญญาณไฟกำลังติดอยู่** (ระบบจะตัดการรับข้อมูลปุ่มกดในจังหวะนี้โดยสิ้นเชิง)
2. **เฟสตอบคำถาม (Input Phase):** เมื่อสัญญาณโจทย์จบลง ระบบจะส่งเสียงสัญญาณและปลดล็อกให้ผู้เล่นทั้งสองฝั่งเริ่มกดตอบคำตอบ
3. **ห้ามกดปุ่มพร้อมกันหลายปุ่ม (Anti-Chording / Single-Press Only):** ผู้เล่นต้องกดทีละปุ่มและปล่อยปุ่มเดิมก่อนกดปุ่มถัดไป หากกดพร้อมกันหรือกดค้างไว้หลายปุ่ม ระบบจะตัดและไม่บันทึกปุ่มที่กดซ้อน
4. **การตัดสินความเร็ว (Tie Breaker):** หากผู้เล่นทั้งสองคนกดถูกต้องเหมือนกันทั้งหมด **ผู้ที่กดถูกต้องครบลำดับได้เร็วที่สุด (มีเวลา Elapsed Time น้อยที่สุด) จะเป็นผู้ชนะและได้ 1 คะแนนในรอบนั้น**
5. **กดผิดแพ้ทันที (Instant Strike Out):** หากผู้เล่นคนใดคนหนึ่งกดผิดลำดับจากโจทย์ จะถูกปรับแพ้ในรอบนั้นทันที และให้คะแนนกับอีกฝั่ง (หากอีกฝั่งกดถูกต้อง)
6. **การหมดเวลา / ตอบผิดคู่ (Timeout / Draw):** ผู้เล่นมีเวลา 15 วินาทีในการตอบคำตอบ หากหมดเวลาโดยไม่มีใครตอบถูก หรือผู้เล่นทั้งสองคนกดผิดทั้งคู่ **รอบนั้นจะถือเป็นโมฆะ (Draw) โดยไม่มีใครได้คะแนน** และระบบจะสุ่มโจทย์ Sequence ใหม่เพื่อเริ่มแข่งรอบนั้นใหม่ทันที
7. **ปุ่มเริ่มเกมใหม่ทั้งหมด (Restart Game):** เมื่อกดปุ่ม **RESTART** บนบอร์ด ระบบจะล้างสถานะเกมปัจจุบันทั้งหมด และรีเซ็ตกลับไปหน้าเริ่มเพื่อให้ผู้เล่นสแกน LINE Login ใหม่อีกครั้งทันที
